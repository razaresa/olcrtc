...
type Config struct {
	...
	MUCDomain   string // optional override; defaults to config.js muc or conference.<host>
	FocusDomain string // optional override; defaults to config.js focus or focus.<host>
}
...
func JoinMUC(ctx context.Context, cfg Config) (*Session, error) {
	...
	domains := resolveJitsiDomains(ctx, cfg)

	conn, err := xmpp.DialWithDomains(ctx, cfg.Host, cfg.Room, cfg.Debug, domains.MUCDomain, domains.FocusDomain)
	if err != nil {
